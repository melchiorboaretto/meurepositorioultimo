...
/* -------------------------------------------------------------------------
 * Estado interno de montagem e Estruturas em Memória
 * ---------------------------------------------------------------------- */
...
/* -------------------------------------------------------------------------
 * IMPLEMENTAÇÃO: Funções Auxiliares de manipulação Estrutural (Inodes/Diretório)
 * ---------------------------------------------------------------------- */
...
{
    const char *id = "Equipe SOFS - Sisop";
    if (name == NULL || size <= 0)
        return -1;
...
int sofs_mount(int partition)
{
    ...
    /* Garante que a Tabela de Arquivos Abertos e Iteradores iniciem purgados */
